  ...
  const renderProfileLinks = () => {
    if (isAuthenticated) {
      return isAdmin ? (
        ...
        <Link to="/myProfile" className="bg-blue-600 text-white font-semibold hover:bg-blue-800 duration-300 px-4 py-2 rounded-md">
          My Profile
        </Link>
      );
    }
      ...
      <div className="flex items-center justify-between container mx-auto">
        ...
        <div className="mx-6">
          <ul className="space-x-6 hidden md:flex">
            ...
            {isAdmin && (
              <Link to="/create-schedule" className="hover:text-blue-600">SCHEDULE</Link>
            )}
          </ul>
      ...
      {show && (
        <div className="bg-white h-screen flex items-center justify-center md:hidden">
          <ul className="flex flex-col space-y-6 text-xl text-center">
            ...
            {isAdmin && (
              <Link to="/create-schedule" className="hover:text-blue-600" onClick={() => setShow(false)}>SCHEDULE</Link>
            )}
